...
export default function MediaUpload({ incidentId, onUploadComplete }: MediaUploadProps) {
  ...
  const requestPermissions = async () => {
    ...
    if (cameraStatus !== 'granted' || libraryStatus !== 'granted') {
      Alert.alert(
        ...
        'Camera and photo library permissions are required to upload evidence.',
        [{ text: 'OK' }]
      )
      return false
    }
  ...
  const pickImage = async (source: 'camera' | 'library') => {
    ...
    try {
      ...
      let result
      if (source === 'camera') {
        // On web, camera might not be available, so fallback to library
        if (Platform.OS === 'web') {
  ...
  const uploadMedia = async (uri: string) => {
    ...
    try {
      // For now, we'll simulate upload by adding the URI to the list
      // In a real implementation, you'd upload to Supabase Storage or another service
      const mediaUrls = [...uploadedMedia, uri]
      
      const result = await EmergencyService.addMediaToAlert(incidentId, [uri])
  ...
  const showUploadOptions = () => {
    ...
    Alert.alert(
      'Upload Evidence',
      Platform.OS === 'web' 
        ? 'Choose how to add evidence (Camera not available on web)'
